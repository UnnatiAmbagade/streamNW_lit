...
if not filtered_data.empty:
    ...
    if not filtered_data_clean.empty:
        center_lat = filtered_data_clean['Latitude'].mean()
        center_lon = filtered_data_clean['Longitude'].mean()
...
with st.sidebar.form(key='admin_form'):
    ...
    submit_button = st.form_submit_button("Add Restaurant")
    ...
    if submit_button:
        if not (-90 <= latitude <= 90):
            ...
            new_restaurant = {
                ...
                'Latitude': latitude,
                'Longitude': longitude,
                'Address': 'Address not provided'  # You might want to add an address input as well
            }
            restaurants = restaurants.append(new_restaurant, ignore_index=True)
            st.success("Restaurant added successfully!")

# Thank you message and completion note
st.sidebar.title("Thank You!")
st.sidebar.write("Thank you for using Taste Tracker. We hope you enjoy discovering great places to eat!")

# Add a thank you note on the main page
st.write("Thank you for exploring Taste Tracker! We hope you find your next favorite dining spot.")
